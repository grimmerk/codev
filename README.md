...
Every term must hold. A space after the colon is fine — `title: ci` reads the same as `title:ci` — except before another operator, since `title: is:live` is two operators rather than a title. An operator with an unreadable value (`after:soon`) is reported under the box and ignored rather than silently matching nothing.
