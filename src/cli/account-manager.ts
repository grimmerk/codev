...
export function readRegistry(): Registry {
  if (!fs.existsSync(REGISTRY_PATH)) {
    return { version: 1, defaultAccount: 'personal', accounts: [] };
  }
...
/** Shell prefix that launches an account (empty for the default account). */
function launchPrefix(account: RegistryAccount | undefined): string {
  if (!account || !account.configDirEnv) return '';
  return `CLAUDE_CONFIG_DIR="${toShellPath(expandHome(account.configDirEnv))}" `;
}
...
export function generateAccountsSh(reg: Registry): string {
  ...
  const L: string[] = [];
  L.push('# CodeV multi-account shell integration — GENERATED FILE (safe to delete).');
  L.push('# Source of truth: ~/.config/codev/accounts.json');
  L.push('# Regenerated by `codev account` / CodeV — manual edits here will be lost.');
  L.push('# To uninstall: `codev account uninstall` (removes the ~/.zshrc block).');
  L.push('#');
  L.push('# Model: one Claude Code config dir per account (CLAUDE_CONFIG_DIR).');
  L.push('#   - DEFAULT account: NO CLAUDE_CONFIG_DIR (config at ~/.claude.json + ~/.claude/).');
  L.push('#   - EXTRA accounts:  CLAUDE_CONFIG_DIR=<dir>, fully self-contained.');
  L.push('#');
  L.push('# Usage:');
  L.push(`#   claude              -> ${defaultLabel || '(none)'} (default account)`);
  L.push('#   claude <account> …  -> that account   (e.g. claude work -r)');
  L.push('#   claude-<account> …  -> equivalent function form');
  L.push('#   claude-whoami       -> which account bare `claude` resolves to here');
  L.push('#   claude-accounts     -> list configured accounts');
  L.push('# All native flags pass through, e.g. `claude work -r`, `claude-work mcp list`.');
  L.push('');
  L.push('# --- per-account launchers (full passthrough via "$@") ---');
  for (const a of accounts) {
    L.push(`claude-${a.label}() { ${launchPrefix(a)}command claude "$@"; }`);
  }
  L.push('');
  L.push('# --- dispatcher: `claude <account> [args...]`, else the default account ---');
  L.push('# `command claude` bypasses this function (no recursion) and any claude alias.');
  L.push('claude() {');
  L.push('  case "$1" in');
  for (const a of accounts) {
    L.push(`    ${a.label}) shift; ${launchPrefix(a)}command claude "$@" ;;`);
  }
  L.push(`    *) ${launchPrefix(defaultAccount)}command claude "$@" ;;`);
  L.push('  esac');
  L.push('}');
  L.push('');
  L.push('# claude-whoami: which account THIS shell resolves to (via CLAUDE_CONFIG_DIR).');
  L.push('claude-whoami() {');
  L.push('  local dir="${CLAUDE_CONFIG_DIR:-$HOME/.claude}" label');
  L.push('  case "$dir" in');
  for (const a of accounts) {
    const marker = a.label === defaultLabel ? ' (default)' : '';
    L.push(`    "${toShellPath(expandHome(a.dir))}") label="${a.label}${marker}" ;;`);
  }
  L.push('    *) label="(unregistered)" ;;');
  L.push('  esac');
  L.push('  echo "codev account     : $label"');
  L.push('  echo "CLAUDE_CONFIG_DIR : ${CLAUDE_CONFIG_DIR:-<unset → ~/.claude>}"');
  L.push('  command claude auth status 2>/dev/null');
  L.push('}');
  L.push('');
  L.push('# claude-accounts: list configured accounts.');
  L.push('claude-accounts() {');
  L.push(`  echo "codev accounts (bare 'claude' = ${defaultLabel || 'none'}):"`);
  for (const a of accounts) {
    const star = a.label === defaultLabel ? '*' : ' ';
    L.push(`  echo "  ${star} ${a.label.padEnd(pad)}   ${toTildePath(a.dir)}"`);
  }
  L.push('  echo "  identity: run claude-whoami, or claude <acct> auth status"');
  L.push('}');
  L.push('');
  return L.join('\n');
}
...
export function addAccount(
  ...
  if (!reg.defaultAccount) reg.defaultAccount = label;
  writeRegistry(reg);
  regenerate(reg);
  return entry;
}
